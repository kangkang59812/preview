...
1. 有自增键情况下：如果不想指定id，则可以写null或者0;或者指定列

   当删除全部n条数据后，在添加数据，会从n+1开始计数；如果删完重启，则从1开始

   当删除x条，再添加数据，从x+1开始,出现断层

   如果最大id值没删除，数据库重启不重启，对于新添加的数据都是没有影响的，还是会延续最大的id然后继续走下去

2. 如果选择了主键，innodb会选择其作为聚簇索引，否则会选择不包含null值的唯一索引；如果没有，则选择内置6字节长的ROWID作为隐含的聚集索引随着行记录的写入而主键递增

3. 使用自增主键，按顺序添加，每一页达到达到装载因子（InnoDB默认为15/16），则开辟一个新的页（节点）。如果使用非自增主键，每次都是随机位置，需要频繁移动数据，缓存从磁盘重新读数据，有开销。

4. 触发器：比如执行了删除操作，会触发某个事件

5. 存储过程：预编译sql语句，创建一次，多次使用。优点：预编译，执行效率高；直接调用减少通讯；安全性高；可重复使用。缺点：移植性差。
